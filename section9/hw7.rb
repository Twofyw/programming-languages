...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    #   *)
    l1 = [seg.x1,seg.x2,seg.y1,seg.y2]
    l2 = [@x1,@x2,@y1,@y2]

    x1start,y1start,x1end,y1end = l1
    x2start,y2start,x2end,y2end = l2
    if real_close(x1start,x1end)
      # (* the segments are on a vertical line *)
      #  (* let segment a start at or below start of segment b *)
      flag = y1start < y2start

      aXstart,aYstart,aXend,aYend = flag ? l1 : l2
      bXstart,bYstart,bXend,bYend = flag ? l2 : l1
